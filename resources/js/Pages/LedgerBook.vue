...
function get_final_balance(account){
    let Balancee = { total_debit:0, total_credit : 0 }
    if (account.entries) {
        account.entries.forEach( (entry,index)=>{
            Balancee.total_debit +=  Number(entry.debit_amount)
            Balancee.total_credit +=  Number(entry.credit_amount)
        }) 
    }
    if (account.children) {
        Balancee = account.children.reduce((accmulation,childe)=>{
...
let Balances = computed(() => {
    ...
    if ( props.account.entries) {
        props.account.entries.forEach((entry,index)=>{
             let Pervious_Balance =(index>0)? Balances_Array[index-1]:0
            Balances_Array.push(Pervious_Balance + Number(entry.debit_amount) - Number(entry.credit_amount) )
        } ) 
    }
...
<template>
     ...
     <tr  v-show="!ShowEntries" tabindex="0" class="border border-slate-400 text-red-900 font-semibold focus:bg-sky-900 focus:text-white divide-x-2 ">
        <td class="p-4 " >
            ...
            <button  v-else type="button" class="p-treetable-toggler p-link" :style="{marginLeft:account.level-1 +'rem'}" style=" visibility: hidden;" tabindex="-1" data-pc-section="rowtoggler" data-pd-ripple="true">
    ...
    <tr v-if="account.entries" class=" " >
        <td v-show="ShowEntries" colspan="5">
            <div  class="w-full">
                <!--  entries table  -->
                <div ref="EntrieTableHeader" tabindex="1" class="w-full flex  bg-white sticky top-24 z-30 text-red-700 focus:bg-sky-900 focus:text-white" >
                ...
                <table     class="border-collapse  table-auto w-full overflow-visible divide-y-2">
                    ...
                    <tr v-for="(entry,index) in account.entries" :key="entry.id" class="bg-gray-50 focus:bg-sky-900 focus:text-white divide-x-2 " tabindex="0" >
                        
                        <td  :ref="(el)=>{is_last_raw(el,index,account.entries.length)}" class="p-4 text-sm">{{ entry.id }}</td>
    ...
    <LedgerBook v-if="ShowChildren" v-for="childe in account.children" :key="childe.id" :account="childe"  >
