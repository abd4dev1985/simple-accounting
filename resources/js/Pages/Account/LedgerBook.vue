...
let EntrieTableHeader= ref(null)
let firstLine= ref(null)
let IsSticky =ref(false)
let TableEntries = ref(null)
let NameCell = ref( null)
let EntryIdCell = ref( null)

let ToogelShowEntries= ()=>{
    if (props.account.children||props.account.entries ) {
        ShowEntries.value = !ShowEntries.value
    }
    if (  EntryIdCell.value) {
    // console.log('NameCell.value.offsetWidth')
    //let NameCellWidth= NameCell.value.clientWidth
   
    // console.log('NameCell')
   // console.log(NameCell.value)
   // console.log(NameCellWidth)

   // console.log('EntryIdCell')
    //console.log(EntryIdCell.value)
    //console.log(EntryIdCell.value.clientWidth)

    
    // EntryIdCell.value.setAttribute("style","width:"+NameCellWidth+"px")
    //console.log('after')
    //console.log(EntryIdCell.value.clientWidth)
...
onMounted(() => {
    ...
    if (EntrieTableHeader.value) {
       ...
       observer.observe(EntrieTableHeader.value)
...
<template>
     ...
     <tr tabindex="0"   ref="firstLine" :class="{'sticky bg-white':IsSticky}"  class=" top-24 z-20 border border-slate-400 text-red-900 font-semibold focus:bg-sky-900 focus:text-white divide-x-2 ">
        <td class="block py-4 px-1 relative "  ref="NameCell">
    ...
    <tr v-if="account.entries" class=" " >
        <td v-show="ShowEntries" colspan="4">
            <div  class="w-full">
                <!--  entries table  -->
                <table  ref="TableEntries"   class="border-collapse  table-auto w-full overflow-visible divide-y-2">
                    <thead  ref="EntrieTableHeader"  class=" dark:bg-gray-700 font-medium dark:border-neutral-500 sticky top-36 z-10">
                        <tr   class="bg-gray-300" >
                            <th scope="col"  ref="EntryIdCell" class=" min-w-[100px] p-4 ">Entry id</th>
                    ...
                    <tr v-for="(entry,index) in account.entries" :key="entry.id" @dblclick="Show_Orgin_Document_of_Entry(entry.entry_id )"
                     class="bg-gray-50 focus:bg-sky-900 focus:text-white divide-x-2 " tabindex="0" >
                        ...
                        <td  class=" block py-4 text-sm">{{ entry.entry_id }}</td>
    ...
    <LedgerBook v-if="account.children" v-for="childe in account.children" :key="childe.id" :account="childe" :level="level+1" >
